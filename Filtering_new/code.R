    ...
    server = shinyServer(function(input, output,session) {
      ...
      full_data <- function() {
        new_data17 <- function() {
          ...
          new_data <- new_data[,-1]
          colnames(new_data) <- c("ID", "type", "import_quantity", "import_price")
          ...
          colnames(new_data2) <- c("ID", "type", "export_quantity", "export_price")
          
          new_data3 <- merge(new_data, new_data2, by = "ID", all = T)
          new_data3 <- new_data3[-c(2,5)]
          new_data3$name <- ""
          new_data3$Year <- "2017"
          new_data3$month <- ""
          
          new_data3 <- new_data3[,c(6,1,7,8,2,3,4,5)]
          
          for (i in 1:length(new_data3$ID)) {
            if(nchar(new_data3$ID[i]) == 5){
              new_data3$ID[i] <- substr(new_data3$ID[i],0,3)
            } else if (nchar(new_data3$ID[i]) == 6) {
              new_data3$ID[i] <- substr(new_data3$ID[i],0,4)
            } else {
              print("wrong")
            }
          }
