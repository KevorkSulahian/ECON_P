...
```{r}
ts_forcaste2 = forecast:::forecast.HoltWinters(ts_forcaste, h= 3)
...
```{r}
ts_arima_forecast = forecast(ts_arima,h = 3)

arima_pred = ts_arima_forecast[4]
arima_pred = arima_pred$mean[3]
...
fit_forecast = forecast(fit3,h=3)
auto.arima_pred = (as.data.frame(fit_forecast))[1][c(3),]
plot(fit_forecast)
# str(fit)
```
...
rownames(df) = "nov-19"
writexl::write_xlsx(df,"economy-activity-indicator-19.xlsx")
```
