...
```{r}
ts_forcaste2 = forecast:::forecast.HoltWinters(ts_forcaste, h= 2)
...
```{r}
year_2018 <- window(ts,2018, c(2018,12))
...
growth_HW <- growth(sum_year_2019,sum(year_2018))
growth_HW 
```
...
```{r}
ts_arima_forecast = forecast(ts_arima,h = 2)
ts_arima_forecast
...
growth_ARIMA <- growth(sum_year_2019,sum(year_2018))
growth_ARIMA 
...
fit_forecast = forecast(fit3,h=2)
plot(fit_forecast)
# str(fit)
```
...
growth_auto.arima <- -growth(sum(year_2018),sum_year_2019)
growth_auto.arima_95_low <- -growth(sum(year_2018),sum_year_2019_low)
growth_auto.arima_95_high <- -growth(sum(year_2018),sum_year_2019_high)
...
rownames(df) = "november-2019"
writexl::write_xlsx(df,"export_total-19.xlsx")
```
